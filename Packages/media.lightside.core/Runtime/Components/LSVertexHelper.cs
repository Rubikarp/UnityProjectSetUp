...
namespace LightSide
{
    public class LSVertexHelper : IDisposable
    {
        ...
        /// <summary>
        /// Rotates the buffered quad by <paramref name="rotateId"/>×90° and mirrors it per <paramref name="flip"/>
        /// (x/y in {0,1}) about <paramref name="center"/>, then re-emits triangles with the winding the rotation
        /// implies. No-op when neither rotation nor flip is requested. Operates in place on the current stream.
        /// </summary>
        public void ApplyRotateFlip(int rotateId, Vector2Int flip, Vector2 center)
        {
            if (rotateId == 0 && flip.x == 0 && flip.y == 0) return;

            var count = currentVertCount;
            UIVertex vert = new UIVertex();
            for (int i = 0; i < count; i++)
            {
                PopulateUIVertex(ref vert, i);
                var pos = vert.position;
                switch (rotateId)
                {
                    case 1: { var x = pos.x; pos.x = -pos.y + center.x; pos.y = x; break; }
                    case 2: { pos.x = -pos.x + center.x; pos.y = -pos.y + center.y; break; }
                    case 3: { var x = pos.x; pos.x = pos.y; pos.y = -x + center.y; break; }
                }
                if (flip.x == 1) pos.x = -(pos.x - center.x);
                if (flip.y == 1) pos.y = -(pos.y - center.y);
                vert.position = pos;
                SetUIVertex(vert, i);
            }

            count = currentIndexCount / 6 * 4;
            ClearTriangles();
            if ((rotateId & 1) == 1)
            {
