...
namespace LightSide
{
    ...
    {
        /// <summary>Canonical full-range expression. Equivalent to <c>Range.All</c>.</summary>
        public const string All = "..";

        /// <summary>
        /// Returns true if the expression parses to a range covering the entire collection,
        /// regardless of which syntactic form was used (<c>".."</c>, <c>"..^0"</c>, <c>"0.."</c>, etc.).
        /// </summary>
        public static bool IsAll(string rangeExpression)
        {
            if (string.IsNullOrEmpty(rangeExpression))
                return false;

            if (!TryParse(rangeExpression, out var range))
                return false;
        ...
        public static bool IsAll(Range range)
        {
