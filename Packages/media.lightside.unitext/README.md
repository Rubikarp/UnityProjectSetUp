...
|----------|--------------|
| Windows | x64, ARM64 |
| Universal Windows Platform (UWP) | x64, ARM64 |
| macOS | x64, Apple Silicon |
| Linux | x64, ARM64 |
| Android | ARMv7, ARM64, x86, x64 |
| iOS | ARM64 (device + simulator) |
| tvOS | ARM64 (device + simulator) |
...
The runtime assembly references `Unity.InputSystem` as an **optional** integration — UPM has no optional-dependency concept, so `com.unity.inputsystem` is deliberately absent from `package.json` dependencies. When the package is not installed Unity skips the unresolved assembly reference and UniText compiles and runs normally. When it is installed, an asmdef `versionDefines` entry activates `UNITEXT_INPUTSYSTEM`, enabling the Input System code paths for pointer/click handling. Native text input (editable text, input fields, IME) is independent of the project's Active Input Handling setting either way.
