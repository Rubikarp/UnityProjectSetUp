...
namespace LightSide
{
    ...
    /// <summary>
    /// Directional predicates over <see cref="JoiningType"/>: two adjacent non-transparent
    /// characters form a cursive connection when the earlier one joins toward the following
    /// and the later one joins toward the preceding (logical order; Transparent characters
    /// between them are skipped per the Unicode Arabic joining algorithm).
    /// </summary>
    internal static class JoiningTypeExtensions
    {
        /// <summary>Whether the character can connect toward the following character in logical order.</summary>
        public static bool JoinsFollowing(this JoiningType jt)
            => jt == JoiningType.DualJoining
                || jt == JoiningType.LeftJoining
                || jt == JoiningType.JoinCausing;

        /// <summary>Whether the character can connect toward the preceding character in logical order.</summary>
        public static bool JoinsPreceding(this JoiningType jt)
            => jt == JoiningType.DualJoining
    ...
    /// <summary>
    /// Unicode Word_Break property values used by the default word-boundary rules in UAX #29.
    /// </summary>
    ...
    /// <summary>Script-classification helpers shared by layout and modifiers.</summary>
    internal static class UnicodeScriptExtensions
    {
        /// <summary>Whether the script joins letters cursively (Arabic-family joining model).</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static bool IsCursiveJoining(this UnicodeScript script)
        {
