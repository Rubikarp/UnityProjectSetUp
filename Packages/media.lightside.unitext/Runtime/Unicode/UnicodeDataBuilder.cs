...
namespace LightSide
{
    ...
    internal struct UnicodeProps
    {
        ...
        public WordBreakProperty wordBreak;
    ...
    internal class UnicodeDataBuilder
    {
        ...
        public void LoadWordBreakProperty(string path)
        {
            using var reader = new StreamReader(path);
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                var codeRangePart = semi[0].Trim();
                var wordBreakPart = semi[1].Trim();

                if (codeRangePart.Length == 0 || wordBreakPart.Length == 0)
                    continue;

                if (!Enum.TryParse(wordBreakPart, out WordBreakProperty wordBreak))
                    throw new InvalidDataException($"Unknown Word_Break value '{wordBreakPart}'.");
                ParseRangeAndApply(codeRangePart, cp => props[cp].wordBreak = wordBreak);
            }
        }
        ...
        public List<WordBreakRangeEntry> BuildWordBreakRangeEntries()
        {
            var result = new List<WordBreakRangeEntry>();

            var currentStart = 0;
            var currentWordBreak = props[0].wordBreak;

            for (var cp = 1; cp <= MaxCodePoint; cp++)
            {
                var wordBreak = props[cp].wordBreak;

                if (wordBreak != currentWordBreak)
                {
                    if (currentWordBreak != WordBreakProperty.Other)
                        result.Add(new WordBreakRangeEntry(currentStart, cp - 1, currentWordBreak));
                    currentStart = cp;
            ...
            if (currentWordBreak != WordBreakProperty.Other)
                result.Add(new WordBreakRangeEntry(currentStart, MaxCodePoint, currentWordBreak));
        ...
        public static List<BracketEntry> BuildBracketEntries(string bidiBracketsPath)
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (pairedPart.Length == 0 || pairedPart.EqualsIgnoreCase("<none>"))
                {
    ...
    {
        /// <summary>
        /// Blob signature 'ULTT'. Must match <see cref="UnicodeDataProvider"/> — bump in lockstep
        ...
        /// <param name="caseMappings">Simple Unicode case mappings.</param>
        /// <param name="wordBreaks">Word_Break property ranges.</param>
        public static void WriteBinary(
            ...
            IReadOnlyList<WordBreakRangeEntry> wordBreaks)
        {
            ...
            var wordBreakOffset = stream.Position;
            writer.Write((uint)wordBreaks.Count);
            foreach (var wb in wordBreaks)
            {
                writer.Write((uint)wb.startCodePoint);
                writer.Write((uint)wb.endCodePoint);
                writer.Write((byte)wb.wordBreak);
                writer.Write((byte)0);
                writer.Write((byte)0);
                writer.Write((byte)0);
            }

            var wordBreakLength = (uint)(stream.Position - wordBreakOffset);
            ...
            writer.Write((uint)wordBreakOffset);
            writer.Write(wordBreakLength);
