...
namespace LightSide
{
    internal static unsafe class FT
    {
        ...
        /// <summary>Serializes FT_Library-level lifecycle. FreeType permits parallel glyph loading on
        /// distinct faces, but FT_New/Done_Face on a shared library must be mutually exclusive.</summary>
        ...
        [DllImport(LibraryName, CallingConvention = Cdecl)] private static extern int ut_ft_get_extended_face_info(IntPtr face, out short capHeight, out short xHeight, out short superscriptYOffset, out short superscriptYSize, out short subscriptYOffset, out short subscriptYSize, out short strikeoutPosition, out short strikeoutSize, out short underlinePosition, out short underlineThickness, out IntPtr familyName, out IntPtr styleName, out short weightClass, out int styleFlags, out short typoAscender, out short typoDescender, out short typoLineGap, out ushort winAscent, out ushort winDescent, out ushort fsSelection);
        ...
        public static bool Initialize()
        {
            ...
            lock (libraryLock)
            {
                ...
                if (initialized)
                    CatZones.native.Meow("[FT] Initialized");
                else
                    CatZones.native.MeowError("[FT] Initialization failed");
        ...
        static FT()
        {
            EditorLifecycle.UnmanagedCleaned += ResetUnloadCounter;
            EditorLifecycle.LibraryShutdown += Shutdown;
        }
    #endif
        ...
        private static void Shutdown()
        {
            lock (libraryLock)
            {
                CatZones.native.Meow($"[FT] Shutdown called, initialized={initialized}, library={(library != IntPtr.Zero)}, pinnedData={pinnedFontData.Count}");
                ...
                initialized = false;
                CatZones.native.Meow("[FT] Shutdown completed");
            }
        }
        ...
        internal static IntPtr LoadFace(IntPtr fontData, int length, int faceIndex = 0)
        {
            if (!initialized || fontData == IntPtr.Zero || length <= 0)
                return IntPtr.Zero;

            lock (libraryLock)
            {
                return ut_ft_new_memory_face(library, (byte*)fontData, length, faceIndex, out var face) == 0
                    ? face
                    : IntPtr.Zero;
            }
        }
        ...
        public static void UnloadFace(IntPtr face)
        {
            if (!initialized)
            {
                CatZones.native.MeowWarn($"[FT] UnloadFace skipped - not initialized! face={face}");
                return;
            }
            if (face == IntPtr.Zero)
            {
                CatZones.native.MeowWarn("[FT] UnloadFace skipped - face is zero");
                return;
            }
        ...
        public static void ResetUnloadCounter()
        {
            ...
            CatZones.native.Meow($"[FT] Total faces unloaded this cycle: {count}");
        }
        ...
        public struct ExtendedFaceInfo
        {
            ...
            public ushort winAscent, winDescent, fsSelection;
            public bool hasOS2;
        }
        ...
        public static ExtendedFaceInfo GetExtendedFaceInfo(IntPtr face)
        {
            ...
            int hasOS2 = ut_ft_get_extended_face_info(face,
                ...
                out result.typoAscender, out result.typoDescender, out result.typoLineGap,
                out result.winAscent, out result.winDescent, out result.fsSelection);
            ...
            else if (bitmap.pixelMode == PIXEL_MODE_GRAY)
            {
                for (int y = 0; y < h; y++)
                {
                    ...
                    for (int x = 0; x < w; x++)
                    {
                        byte gray = row[x];
                        int dstIdx = dstOffset + x * 4;
                        pixelBuffer[dstIdx + 0] = gray;
                        pixelBuffer[dstIdx + 1] = gray;
                        pixelBuffer[dstIdx + 2] = gray;
            ...
            else if (bitmap.pixelMode == PIXEL_MODE_MONO)
            {
                for (int y = 0; y < h; y++)
                {
                    ...
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        int dstIdx = dstOffset + x * 4;
                        pixelBuffer[dstIdx + 0] = val;
                        pixelBuffer[dstIdx + 1] = val;
                        pixelBuffer[dstIdx + 2] = val;
        ...
        /// <summary>Returns whether the face contains a COLR table.</summary>
        public static bool HasCOLRTable(IntPtr face)
        {
            if (!initialized || face == IntPtr.Zero) return false;
            return ut_colr_debug_glyph_paint(face, 0, out _, out _, out _) != 0;
        }
                ...
                else if (bitmap.pixelMode == PIXEL_MODE_GRAY)
                {
                    for (int y = 0; y < h; y++)
                    {
                        ...
                        for (int x = 0; x < w; x++)
                        {
                            byte gray = row[x];
                            int dstIdx = dstOffset + x * 4;
                            dstPtr[dstIdx + 0] = gray;
                            dstPtr[dstIdx + 1] = gray;
                            dstPtr[dstIdx + 2] = gray;
