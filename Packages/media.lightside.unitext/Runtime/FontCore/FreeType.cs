...
namespace LightSide
{
    ...
    internal static class FreeType
    {
        ...
        private static FontSource currentFontSource;
        private static FreeTypeFace currentFaceOwner;
        ...
        public static int GetNumFaces(byte[] fontData)
            => fontData == null || fontData.Length == 0
                ? 0
                : GetNumFaces(new ArrayFontSource(fontData));

        internal static int GetNumFaces(FontSource fontSource)
        {
            if (!FT.Initialize() || fontSource == null)
                return 0;

            using var tempFace = FreeTypeFace.TryCreate(fontSource, 0);
            if (tempFace == null)
                return 0;

            var info = FT.GetFaceInfo(tempFace.Pointer);
        ...
        {
            return GetNumFaces(FontFileCache.OpenSnapshot(fontPath));
        }

        public static bool LoadFontFromPath(string fontPath, int faceIndex = 0)
        {
            return LoadFontFromSource(FontFileCache.OpenSnapshot(fontPath), faceIndex);
        }
    #endif
        ...
        public static bool LoadFontFromData(byte[] fontData, int faceIndex = 0)
            => fontData != null && fontData.Length != 0
               && LoadFontFromSource(new ArrayFontSource(fontData), faceIndex);

        internal static bool LoadFontFromSource(FontSource fontSource, int faceIndex = 0)
        {
            if (!FT.Initialize())
                return false;
            ...
            var owner = FreeTypeFace.TryCreate(fontSource, faceIndex);
            if (owner == null)
            {
            ...
            currentFaceOwner = owner;
            currentFace = owner.Pointer;
            currentFontSource = fontSource;
        ...
        internal static bool IsCurrent(FontSource fontSource, int faceIndex = 0)
            => currentFace != IntPtr.Zero
               && ReferenceEquals(currentFontSource, fontSource)
               && currentFaceIndex == faceIndex;

        internal static void UnloadIfCurrent(FontSource fontSource, int faceIndex = 0)
        {
            if (IsCurrent(fontSource, faceIndex)) DisposeFace();
        }
        ...
        {
            return GetFaceInfo(currentFace, currentFaceIndex, ref cachedFixedSizes);
        }

        internal static FaceInfo GetFaceInfo(IntPtr face)
        {
            int[] fixedSizes = null;
            return GetFaceInfo(face, -1, ref fixedSizes);
        }

        private static FaceInfo GetFaceInfo(IntPtr face, int faceIndex, ref int[] fixedSizes)
        {
            var info = new FaceInfo { isValid = false };
            if (face == IntPtr.Zero) return info;
            ...
            info.faceIndex = faceIndex >= 0 ? faceIndex : ftInfo.faceIndex;
            ...
            {
                fixedSizes ??= FT.GetAllFixedSizes(face);
                info.availableSizes = fixedSizes;
            }
        ...
        private static void DisposeFace()
        {
            if (currentFace != IntPtr.Zero)
            {
                currentFaceOwner?.Dispose();
                currentFaceOwner = null;
                currentFace = IntPtr.Zero;
            }

            currentFontSource = null;
            currentPixelSize = 0;
            currentFaceIndex = 0;
            cachedFixedSizes = null;
        }
    ...
    #if UNITY_EDITOR
        static FreeType() => EditorLifecycle.UnmanagedCleaning += Dispose;
    #endif
