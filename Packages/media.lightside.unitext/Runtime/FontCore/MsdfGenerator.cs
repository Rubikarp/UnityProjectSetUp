...
namespace LightSide
{
    ...
    internal unsafe struct MsdfJob : IJobParallelFor
    {
            ...
            MonoSegment* monoSegs, int monoCapacity)
        {
            ...
            int monoCount = YMonotoneSplit(segOff, segCount, monoSegs, monoCapacity);
            ComputeWinding(monoSegs, monoCount, tileSize, scale, offsetX, offsetY,
                signGrid, rxMin, ryMin, rxMax, ryMax);

            float invScale = 1f / scale;
            bool resolved = segCount > 0 &&
                (segments[segOff].rasterFlags & GlyphCurveCache.Segment.FlagResolved) != 0;
            ...
            for (int ch = 0; ch < 3; ch++)
            {
                ...
                for (int i = 0; i < segCount; i++)
                {
                    ...
                    SeedQuadratic(s.p0x, s.p0y, s.p1x, s.p1y, s.p2x, s.p2y,
                        ...
                        s.cornerFlags, channelBit, signGrid, resolved);
                }
            ...
            {
                ulong* dstRow = tileBase + ey * tileSize;
                ...
                {
                    int pi = ey * tileSize + ex;
                    int bi = pi * 3;
                    int vi = pi * 4;
                    float vx = vecGrid[vi], vy = vecGrid[vi + 1];
                    float dist = math.sqrt(vx * vx + vy * vy);
                    float trueSdf = math.saturate((signGrid[pi] != 0 ? -dist : dist) * invSpread + 0.5f);

                    dstRow[ex] = (ulong)math.f32tof16(msdfBuf[bi])
                               | ((ulong)math.f32tof16(msdfBuf[bi + 1]) << 16)
                               | ((ulong)math.f32tof16(msdfBuf[bi + 2]) << 32)
                               | ((ulong)math.f32tof16(trueSdf) << 48);
                }
        ...
        private void SeedQuadratic(float ax, float ay, float bx, float by, float cx, float cy,
            ...
            byte cornerFlags, int channelBit, byte* signGrid, bool resolved)
        {
            ...
            int rxMin, int ryMin, int rxMax, int ryMax)
        {
            ...
            for (int y = ryMin; y <= ryMax; y++)
            {
                ...
                for (int x = rxMax - 1; x >= rxMin; x--)
                {
                    ...
                    float curVy = vecGrid[idx + 1];
                    float curD2 = curVx * curVx + curVy * curVy;
                    CheckProp(idx + stride, ref curVx, ref curVy, ref curD2, 1f, 0f, vecGrid, idx);
                    vecGrid[idx] = curVx;
                    vecGrid[idx + 1] = curVy;
                }
            }
        ...
        private void ClearTile(ulong* tileBase, int tileSize)
        {
            ulong h1 = (ulong)math.f32tof16(1f);
            ulong white = h1 | (h1 << 16) | (h1 << 32) | (h1 << 48);
            for (int y = 0; y < tileSize; y++)
            {
                ulong* dstRow = tileBase + y * tileSize;
