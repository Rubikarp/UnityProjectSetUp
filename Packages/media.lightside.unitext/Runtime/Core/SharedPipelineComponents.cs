...
namespace LightSide
{
    ...
    internal static class SharedPipelineComponents
    {
        ...
        private static readonly Lazy<WordSegmentationProcessor> wordSegmentation = new(() =>
        {
            ...
            UniTextSettings.Changed += (in StateChange change) =>
            {
                if (!UniTextSettings.Affects(in change, UniTextSettings.Members.Dictionaries)) return;
                processor.Clear();
                InitializeSegmenters(processor);
            };
            return processor;
            ...
            for (var i = 0; i < dicts.Count; i++)
            {
                ...
                if (!dict.IsValid)
                    throw new InvalidOperationException(
                        $"Word segmentation dictionary '{dict.name}' has invalid compiled data or target script.");
