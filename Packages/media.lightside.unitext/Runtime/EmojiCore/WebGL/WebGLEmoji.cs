...
namespace LightSide
{
    ...
    {
        private const int MaxRenderedDimension = 4096;
        ...
        {
            if (!IsSupported || codepoints.IsEmpty || pixelSize <= 0)
                return true;
        ...
        private static int MinPixelBufferSize
        {
            get
            {
                int size = EmojiFont.Instance?.ColorPixelSize ?? EmojiFont.DefaultSize;
                return size * size * 4;
            }
        }
        ...
        public static bool TryRenderEmoji(ReadOnlySpan<int> codepoints, int pixelSize, out RenderedEmoji result)
        {
            ...
            GCHandle cpHandle = default;
            GCHandle metricsHandle = default;
            IntPtr pixelsPtr = IntPtr.Zero;
            try
            {
                cpHandle = GCHandle.Alloc(tempCodepointBuffer, GCHandleType.Pinned);
                metricsHandle = GCHandle.Alloc(metricsBuffer, GCHandleType.Pinned);
                IntPtr cpPtr = cpHandle.AddrOfPinnedObject();
                IntPtr metricsPtr = metricsHandle.AddrOfPinnedObject();

                pixelsPtr = JS_BrowserEmoji_RenderEmoji(cpPtr, codepoints.Length, pixelSize, metricsPtr);
                ...
                if (width <= 0 || height <= 0 || width > MaxRenderedDimension
                    || height > MaxRenderedDimension || pixelsPtr == IntPtr.Zero)
                {
                ...
                int dataSize = checked(width * height * 4);
            ...
            finally
            {
                ...
                {
                    if (cpHandle.IsAllocated) cpHandle.Free();
                    if (metricsHandle.IsAllocated) metricsHandle.Free();
                }
            }
        }
        ...
        [ThreadStatic] private static int[] batchOriginalIndices;
        ...
        public static bool TryRenderEmojiBatch(List<uint> hashes, int pixelSize, out BatchRenderResult result)
        {
            ...
            if (!IsSupported || hashes == null || hashes.Count == 0 || pixelSize <= 0)
                return false;
            ...
            {
                CatZones.emoji.MeowWarn($"[WebGLEmoji] TryRenderEmojiBatch: No sequences found for {count} glyphIds. Cache size: {sequences.Count}");
                return false;
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                batchOriginalIndices[validCount] = i;
            ...
            GCHandle cpHandle = default;
            GCHandle offsetsHandle = default;
            GCHandle lengthsHandle = default;
            GCHandle metricsHandle = default;
            GCHandle pixelOffsetsHandle = default;
            IntPtr pixelsPtr = IntPtr.Zero;
            bool transferred = false;
            try
            {
                cpHandle = GCHandle.Alloc(batchCodepoints, GCHandleType.Pinned);
                offsetsHandle = GCHandle.Alloc(batchOffsets, GCHandleType.Pinned);
                lengthsHandle = GCHandle.Alloc(batchLengths, GCHandleType.Pinned);
                metricsHandle = GCHandle.Alloc(batchMetrics, GCHandleType.Pinned);
                pixelOffsetsHandle = GCHandle.Alloc(batchPixelOffsets, GCHandleType.Pinned);
                pixelsPtr = JS_BrowserEmoji_RenderEmojiBatch(
                ...
                if (pixelsPtr == IntPtr.Zero)
                    CatZones.emoji.MeowWarn($"[WebGLEmoji] JS_BrowserEmoji_RenderEmojiBatch returned null for {validCount} emoji");

                transferred = pixelsPtr != IntPtr.Zero;
            ...
            finally
            {
                try
                {
                    if (!transferred && pixelsPtr != IntPtr.Zero)
                        JS_BrowserEmoji_FreeRenderedData();
                }
                finally
                {
                    ...
                    if (lengthsHandle.IsAllocated) lengthsHandle.Free();
                    if (metricsHandle.IsAllocated) metricsHandle.Free();
                    if (pixelOffsetsHandle.IsAllocated) pixelOffsetsHandle.Free();
                }
            }
        }
        ...
        private static void EnsureBatchBuffers(int count, int totalCodepoints)
        {
            ...
            if (batchOriginalIndices == null || batchOriginalIndices.Length < count)
                batchOriginalIndices = new int[Math.Max(count, InitialBatchCapacity)];
        }
        ...
        private static int GetTotalPixelSize(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (w < 0 || h < 0 || w > MaxRenderedDimension || h > MaxRenderedDimension)
                    throw new InvalidOperationException("Browser emoji batch returned invalid dimensions.");
                total = checked(total + checked(w * h * 4));
            }
            return total;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int GetBatchOriginalIndex(int index) => batchOriginalIndices[index];
