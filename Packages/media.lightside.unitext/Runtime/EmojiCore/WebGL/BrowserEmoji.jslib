mergeInto(LibraryManager.library, {
    ...
    $BrowserEmoji: {
        ...
        copyPremultipliedRect: function(source, sourceWidth, sourceX, sourceY, width, height) {
            var result = new Uint8Array(width * height * 4);
            for (var y = 0; y < height; y++) {
                for (var x = 0; x < width; x++) {
                    var srcIdx = ((sourceY + y) * sourceWidth + sourceX + x) * 4;
                    var dstIdx = (y * width + x) * 4;
                    var alpha = source[srcIdx + 3];
                    result[dstIdx] = (source[srcIdx] * alpha + 127) / 255;
                    result[dstIdx + 1] = (source[srcIdx + 1] * alpha + 127) / 255;
                    result[dstIdx + 2] = (source[srcIdx + 2] * alpha + 127) / 255;
        ...
        renderEmoji: function(emojiStr, pixelSize) {
            ...
            var trimmedPixels = this.copyPremultipliedRect(
                pixels, width, minX, minY, trimmedWidth, trimmedHeight);
    ...
    JS_BrowserEmoji_RenderEmojiBatch: function(codepointsPtr, offsetsPtr, lengthsPtr, count, pixelSize, outMetricsPtr, outPixelOffsetsPtr) {
        ...
        while (processed < count) {
            ...
            for (var i = 0; i < batchSize; i++) {
                ...
                var trimmedWidth = maxX - minX + 1;
                var trimmedHeight = maxY - minY + 1;
                var trimmedPixels = BrowserEmoji.copyPremultipliedRect(
                    pixels, canvasW, cellX + minX, cellY + minY, trimmedWidth, trimmedHeight);
