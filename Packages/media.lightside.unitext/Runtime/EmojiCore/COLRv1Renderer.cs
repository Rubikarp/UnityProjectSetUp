...
namespace LightSide
{
    ...
    /// </summary>
    internal sealed class COLRv1Renderer
    {
        ...
        internal COLRv1Renderer(IntPtr ftFace)
        {
            face = ftFace;
            InitializePalette();
        }
        ...
        /// </summary>
        internal bool TryRenderGlyph(uint glyphIndex, int pixelSize, FT.FT_OpaquePaint rootPaint,
            out byte[] rgbaPixels, out int width, out int height,
            out float bearingX, out float bearingY, out float renderScale)
        {
            ...
            {
                CatZones.emoji.MeowError("[COLRv1] Failed to create Blend2D image");
                return false;
            }
            ...
            if (blCtx == IntPtr.Zero)
            {
                ...
                CatZones.emoji.MeowError("[COLRv1] Failed to create Blend2D context");
                return false;
            }
            ...
            try
            {
                ...
                {
                    CatZones.emoji.MeowError("[COLRv1] Failed to get image data");
                    return false;
                }

                int pixelDataSize = renderWidth * renderHeight * 4;
                rgbaPixels = ArrayPool<byte>.Rent(pixelDataSize);
                try
                {
        ...
        private void RenderPaint(FT.FT_OpaquePaint opaquePaint)
        {
            ...
            {
                CatZones.emoji.MeowWarn("[COLRv1] Maximum paint depth exceeded");
                return;
            }
            ...
            switch (format)
            {
                ...
                default:
                    CatZones.emoji.MeowWarnFormat("[COLRv1] Unknown paint format: {0}", format);
                    break;
            }
            paintDepth--;
        }
